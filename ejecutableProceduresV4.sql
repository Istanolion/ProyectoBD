...
CREATE OR REPLACE PROCEDURE spPrestamo(
	...
	BEGIN
		...
		VALUES(vIdPrestamo,NULL,vIdMaterial,vIdLector,
...
CREATE OR REPLACE PROCEDURE spDevol(
	...
	BEGIN
		...
		--DEVOLUCION DEL MATERIAL AUNQUE ESTE GENERE UNA MULTA O NO
		UPDATE prestamo
		SET fechaDevl = SYSDATE
		WHERE idPrestamo = vIdPrestamo;
		DBMS_OUTPUT.PUT_LINE('SE REALIZO LA DEVOLUCION DEL MATERIAL DEL PRESTAMO '||vIdPrestamo);
		--INTENTO DE BORRADO DEL PRESTAMO
...
CREATE OR REPLACE PROCEDURE spResello(
	...
	BEGIN
		...
		--TAMBIEN VERIFICA QUE NO SE TENGA EL LIMITE DE REFRENDOS
			...
			fechaVen = SYSDATE + vDiasTipoLector,
			numRese = numRese+1
			WHERE idPrestamo = vIdPrestamo;
			DBMS_OUTPUT.PUT_LINE('SE REALIZO UN RESELLO EN EL PRESTAMO '||vIdPrestamo);
...
CREATE OR REPLACE PROCEDURE spPagMulta(
	...
	--SE ELIMINA EL REGISTRO DE MULTA
		...
		DBMS_OUTPUT.PUT_LINE('BORRARA EL PRESTAMO CADUCO '||vIdPrestamo);
		DELETE FROM prestamo
		WHERE idPrestamo = vIdPrestamo;
END spPagMulta;
/
...
CREATE OR REPLACE PROCEDURE spMulta(
	...
	SELECT fechaVen INTO vFechaDev
	FROM prestamo
	WHERE idPrestamo = vIdPrestamo;

	vMonto := ftMulta(vFechaDev);
	--INGRESO DE LA MULTA
	INSERT INTO multa
	(idPrestamo,monto,diasRetraso)	
	VALUES(vIdPrestamo,vMonto,TRUNC(TO_NUMBER(SYSDATE-vFechaDev)));
